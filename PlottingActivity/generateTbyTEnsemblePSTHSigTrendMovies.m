function generateTbyTEnsemblePSTHSigTrendMovies
...
for iDataset = 1:length(datasets)
    ...
    sav_dir = [datasets{iDataset} '\EnsemblePSTHs\TransitionMovies\SigmoidApproximation'];
    mkdir(sav_dir)
    ...
    edges = -1000:50:1000;
    evalEdges = linspace(-1,1,length(edges)-1);
        ...
        for iSU = 1:nSU
            ...
            if ismember(iSU,pref270_br)|| ismember(iSU,pref90_br)
                ...
                if ~isempty(piece)
                    ...
                    temp = histcounts(piece,edges);
                    [p,stats] = sigm_fit(evalEdges,temp,[],[],0);
                    sdf_br_s270TO90{iSwitch}(elec,:) = normalise(stats.ypred);
                else
        ...
        for iSU = 1:nSU
            ...
            if ismember(iSU,pref270_br)|| ismember(iSU,pref90_br)
                ...
                if ~isempty(piece)
                    ...
                    temp = histcounts(piece,edges);
                    [p,stats] = sigm_fit(evalEdges,temp,[],[],0);
                    sdf_br_s90TO270{iSwitch}(elec,:) = normalise(stats.ypred);
                else
        ...
        for iSU = 1:nSU
            ...
            if ismember(iSU,pref270_pa)|| ismember(iSU,pref90_pa)
                ...
                if ~isempty(piece)
                    ...
                    temp = histcounts(piece,edges);
                    [p,stats] = sigm_fit(evalEdges,temp,[],[],0);
                    sdf_pa_s270TO90{iSwitch}(elec,:) = normalise(stats.ypred);
                else
        ...
        for iSU = 1:nSU
            ...
            if ismember(iSU,pref270_pa)|| ismember(iSU,pref90_pa)
                ...
                if ~isempty(piece)
                    ...
                    temp = histcounts(piece,edges);
                    [p,stats] = sigm_fit(evalEdges,temp,[],[],0);
                    sdf_pa_s90TO270{iSwitch}(elec,:) = normalise(stats.ypred);
                else
    ...
    for iSwitch = 1:length(sdf_br_s270TO90)
        ...
        pref270Traces = normalise(smooth(nanmean(counts_br_s270TO90{iSwitch}(chan2elec(pref270_br),:),1)));
        pref90Traces = normalise(smooth(nanmean(counts_br_s270TO90{iSwitch}(chan2elec(pref90_br),:),1)));
        ...
        for iBin = 5:length(edges)-1-4
            ...
            subplot(2,2,[3 4])
            hold all
            plot(edges(5:iBin)/1000,pref270Traces(5:iBin),'-g','LineWidth',2)
            plot(edges(5:iBin)/1000,pref90Traces(5:iBin),'-r','LineWidth',2)
            vline(0,'--b')
            drawnow
            vline(0,'--b')
    ...
    for iSwitch = 1:length(sdf_br_s90TO270)
        ...
        pref270Traces = normalise(smooth(nanmean(counts_br_s90TO270{iSwitch}(chan2elec(pref270_br),:),1)));
        pref90Traces = normalise(smooth(nanmean(counts_br_s90TO270{iSwitch}(chan2elec(pref90_br),:),1)));
    ...
    for iSwitch = 1:length(sdf_pa_s270TO90)
        ...
        pref270Traces = normalise(smooth(nanmean(counts_pa_s270TO90{iSwitch}(chan2elec(pref270_pa),:),1)));
        pref90Traces = normalise(smooth(nanmean(counts_pa_s270TO90{iSwitch}(chan2elec(pref90_pa),:),1)));
    ...
    for iSwitch = 1:length(sdf_pa_s90TO270)
        ...
        pref270Traces = normalise(smooth(nanmean(counts_pa_s90TO270{iSwitch}(chan2elec(pref270_pa),:),1)));
        pref90Traces = normalise(smooth(nanmean(counts_pa_s90TO270{iSwitch}(chan2elec(pref90_pa),:),1)));
