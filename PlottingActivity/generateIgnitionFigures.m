function generateIgnitionFigures(dataPath,binWidth)
...
meanAmplitudePreBR_270TO90 = zeros(nSessions,nBins-1,nChans);
...
meanAmplitudePreBR_90TO270 = zeros(nSessions,nBins-1,nChans);
...
meanAmplitudePrePA_270TO90 = zeros(nSessions,nBins-1,nChans);
...
meanAmplitudePrePA_90TO270 = zeros(nSessions,nBins-1,nChans);
...
%     % open videofile
%     
%     filename = ['StaticBins_PreBR_LowFreq_IgnitedChannels_Session' '_' num2str(binWidth) '_ms' num2str(iDataset) '.avi'];
%     cd(dataPath{3})
...
%     close all;
%     
%     filename = ['StaticBins_PrePA_LowFreq_IgnitedChannels_Session' '_' num2str(binWidth) '_ms' num2str(iDataset) '.avi'];
%     cd(dataPath{3})
%     v = VideoWriter(filename);
%     v.FrameRate = 7;
%     open(v);
...
%     % open videofile
%     
%     filename = ['StaticBins_PrePM_LowFreq_IgnitedChannels_Session' '_' num2str(binWidth) '_ms' num2str(iDataset) '.avi'];
%     cd(dataPath{3})
%     v = VideoWriter(filename);
%     v.FrameRate = 7;
%     open(v);
...
fn = ['Ignition_StaticBins_' num2str(binWidth) 'ms.mat'];
save(fn);

figure(2)
plot(bins(1:end-1),smooth(nanmean(posChansBR_270TO90,1)),'-r','LineWidth',2)
hold on
plot(bins(1:end-1),smooth(nanmean(posChansBR_90TO270,1)),'-b','LineWidth',2)
plot(bins(1:end-1),smooth(nanmean(posChansPA_270TO90,1)),'--r','LineWidth',1)
hold on
plot(bins(1:end-1),smooth(nanmean(posChansPA_90TO270,1)),'--b','LineWidth',1)
xlabel('static bins approaching switch')
ylabel('Number of channels Activated per Bin')
title('Grand Average Ignition')
grid on
legend('PreBR - 270TO90','PreBR - 90TO270','PrePA - 270TO90','PrePA - 90TO270')
pause; clf;
