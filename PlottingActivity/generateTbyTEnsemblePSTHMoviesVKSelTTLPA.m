function generateTbyTEnsemblePSTHMoviesVKSelTTLPA
...
for iDataset = 1:length(datasets)
    ...
    trialInformation = collectTrialInformationHayo(params,qnxfile,eventfile,'pfc','spikes',0);
    [spikingActivity] = collectCleanDominancesSpikesHayo(params,jMUspikes,durs);
        ...
        for iSU = 1:nSU
            ...
            if ismember(iSU,pref270)|| ismember(iSU,pref90)
                ...
                if ~isempty(piece)
                    ...
                    sdf_pa_s270TO90{iSwitch}(elec,:) = temp;
                else
    ...
    for iSwitch = 1:length(sdf_pa_s270TO90)
        ...
        v = VideoWriter(['TTLPA_s270TO90_Transition_' num2str(iSwitch) '.avi']);
        v.FrameRate = 5;
        open(v);
        figure;
        ...
        for iBin = 3:length(edges)-2
            ...
            subplot(2,2,[3 4])
            hold all
            plot(edges(3:iBin),pref270Traces(3:iBin,1),'Color',[0.56,0.82,0.31],'LineWidth',2)
            plot(edges(3:iBin),pref90Traces(3:iBin,1),'Color',[0.91,0.7,0.7],'LineWidth',2)
            vline(0,'--b')
            drawnow
            vline(0,'--b')
    ...
    for iSwitch = 1:length(sdf_pa_s90TO270)
        ...
        v = VideoWriter(['TTLPA_s90TO270_Transition_' num2str(iSwitch) '.avi']);
        v.FrameRate = 5;
        open(v);
        figure;
