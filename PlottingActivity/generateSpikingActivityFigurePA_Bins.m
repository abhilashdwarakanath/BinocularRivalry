function generateSpikingActivityFigurePA_Bins
...
for iDataset = 1:nDatasets
    ...
    [idx] = (events(:,1) == 6); % these two events mark the beginning of a dominance
    
    startTimes = ceil(events(idx,2)*60); % transform to samples
    
    switches_sel90_s270TO90 = zeros(length(pref90_pa),length(startTimes),length(edges)); % initialise collection variable
    ...
    for iTransitions = 1:length(startTimes)
        ...
        for iChan = 1:length(pref90_pa)
            ...
            if ~isempty(temp)
                ...
                psth = histc(temp,edges);
                switches_sel90_s270TO90(iChan,iTransitions,:) = psth;
                clear temp; clear st; clear tmp;
                
            else
                switches_sel90_s270TO90(iChan,iTransitions,:) = zeros(1,length(edges));
    ...
    [idx] = (events(:,1) == 5); % these two events mark the beginning of a dominance
    
    startTimes = ceil(events(idx,2)*60); % transform to samples
    
    switches_sel270_s90TO270 = zeros(length(pref270_pa),length(startTimes),length(edges)); % initialise collection variable
    ...
    for iTransitions = 1:length(startTimes)
        ...
        for iChan = 1:length(pref270_pa)
            ...
            if ~isempty(temp)
                ...
                switches_sel270_s90TO270(iChan,iTransitions,:) = zeros(1,length(edges));
...
subplot(2,2,1)
imagesc(edges(6:46)./1000,1:size(allSwitches_sel90_s270TO90,1),allSwitches_sel90_s270TO90(:,6:46)-mean(mean(allSwitches_sel90_s270TO90(:,6:46))));
colormap jet
vline(0,'--w')
...
AX.CLim = [0 5];
box off
title('270 to 90 Switches PA')

subplot(2,2,2)
plot(edges(6:46)./1000,nanmean(allSwitches_sel90_s270TO90(:,6:46),1),'LineWidth',2)
xlabel('time relative to switch [s]')
ylabel('mean spikes/s')
box off
vline(0,'--k')
title('Mean PSTH 270 to 90 Switches PA')

subplot(2,2,3)
imagesc(edges(6:46)./1000,1:size(allSwitches_sel270_s90TO270,1),allSwitches_sel270_s90TO270(:,6:46)-mean(mean(allSwitches_sel270_s90TO270(:,6:46))));
colormap jet
vline(0,'--w')
xlabel('time relative to switch [s]')
ylabel('# of switches')
...
AX.CLim = [0 5];
box off
title('90 to 270 Switches PA')

subplot(2,2,4)
plot(edges(6:46)./1000,nanmean(allSwitches_sel270_s90TO270(:,6:46),1),'-r','LineWidth',2)
xlabel('time relative to switch [s]')
ylabel('mean spikes/s')
box off
vline(0,'--k')
title('Mean PSTH 90 to 270 Switches PA')
