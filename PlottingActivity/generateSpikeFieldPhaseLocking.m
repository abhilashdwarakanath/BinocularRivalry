function generateSpikeFieldPhaseLocking
...
for iBin = 1:4
    ...
    for iDataset = 1:length(datasets)
        ...
        post_BR_ga_dominant = [post_BR_ga_dominant; circ_mean((SFC(iBin).phi_BR_sel90_s270TO90_Post),[],3);circ_mean((SFC(iBin).phi_BR_sel270_s90TO270_Post),[],3)];
        post_BR_ga_suppressed = [post_BR_ga_suppressed; circ_mean((SFC(iBin).phi_BR_sel90_s90TO270_Post),[],3);circ_mean((SFC(iBin).phi_BR_sel270_s270TO90_Post),[],3)];
        
        pre_BR_ga_dominant = [pre_BR_ga_dominant; circ_mean((SFC(iBin).phi_BR_sel90_s90TO270_Pre),[],3);circ_mean((SFC(iBin).phi_BR_sel270_s270TO90_Pre),[],3)];
        pre_BR_ga_suppressed = [pre_BR_ga_suppressed; circ_mean((SFC(iBin).phi_BR_sel90_s270TO90_Pre),[],3);circ_mean((SFC(iBin).phi_BR_sel270_s90TO270_Pre),[],3)];
        
        post_PA_ga_dominant = [post_PA_ga_dominant; circ_mean((SFC(iBin).phi_PA_sel90_s270TO90_Post),[],3);circ_mean((SFC(iBin).phi_PA_sel270_s90TO270_Post),[],3)];
        post_PA_ga_suppressed = [post_PA_ga_suppressed; circ_mean((SFC(iBin).phi_PA_sel90_s90TO270_Post),[],3);circ_mean((SFC(iBin).phi_PA_sel270_s270TO90_Post),[],3)];
        
        pre_PA_ga_dominant = [pre_PA_ga_dominant; circ_mean((SFC(iBin).phi_PA_sel90_s90TO270_Pre),[],3);circ_mean((SFC(iBin).phi_PA_sel270_s270TO90_Pre),[],3)];
        pre_PA_ga_suppressed = [pre_PA_ga_suppressed; circ_mean((SFC(iBin).phi_PA_sel90_s270TO90_Pre),[],3);circ_mean((SFC(iBin).phi_PA_sel270_s90TO270_Pre),[],3)];
    end
...
% Get the difference between dominant and suppressed
    ...
    if iBin == 1
        ...
        titleText = ['Phi: Selective Sites vs Global LFP - Full Period'];
        fileNameText = ['cutoff_raw_Phi_Selective_Units_vs_Global_LFP_Full_Period.fig'];
        
    elseif iBin == 2
        
        titleText = ['Phi: Selective Sites vs Global LFP. Pre: -1:-0.5. Post: 0:0.5'];
        fileNameText = ['cutoff_raw_Phi_Selective_Units_vs_Global_LFP_Pre_-1_-0.5_Post_0_0.5.fig'];
        
    elseif iBin == 3
        
        titleText = ['Phi: Selective Sites vs Global LFP. Pre: -0.5:0. Post: 0.5:1'];
        fileNameText = ['cutoff_raw_Phi_Selective_Units_vs_Global_LFP_Pre_-0.5_0_Post_0.5_1.fig'];
        
    elseif iBin == 4
        
        titleText = ['Phi: Selective Sites vs Global LFP. Pre: -1:-0.25 Post: 0.25:1'];
        fileNameText = ['cutoff_raw_Phi_Selective_Units_vs_Global_LFP_Pre_-1_0.25_Post_0.25_1.fig'];
...
%     for iFreq = 1:size(pre_PA_ga_dominant,2)
%         [sigFreqs.p_dom_vs_sup_pre_PA(iFreq)] = circ_wwtest(pre_PA_ga_dominant(:,iFreq),pre_PA_ga_suppressed(:,iFreq));
%     end
...
%     
%     [sigFreqs.p_dom_vs_sup_post_PA] = circ_wwtest(post_PA_ga_dominant,post_PA_ga_suppressed);
...
%     [sigFreqs.p_dom_vs_sup_pre_BR] = circ_wwtest(pre_BR_ga_dominant,pre_BR_ga_suppressed);
...
%     [sigFreqs.p_dom_vs_sup_post_BR] = circ_wwtest(post_BR_ga_dominant,post_BR_ga_suppressed);
